...
class Database:
    ...
    async def init_indexes(self):
        if self.database is not None:
            ...
            await self.database["tasks"].create_index([("status", 1), ("due_date", 1)])
            await self.database["tasks"].create_index([("assigned_to", 1), ("status", 1)])
            await self.database["tasks"].create_index([("project_id", 1), ("status", 1)])
